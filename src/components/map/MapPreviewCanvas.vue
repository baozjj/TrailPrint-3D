...
import { metersPerPixel, projectPoint, zoomToFitBounds } from '@/utils/map-projection'
...
function fitTrackInView(): void {
  ...
  config.value.mapCrop.mapZoom = zoomToFitBounds(bounds, rect.width, rect.height)
  draw()
}
...
function drawTerrainBackground(ctx: CanvasRenderingContext2D, w: number, h: number): void {
...
function drawCropMask(ctx: CanvasRenderingContext2D, w: number, h: number): void {
...
function onWheel(e: WheelEvent): void {
  ...
  config.value.mapCrop.mapZoom = Math.min(18, Math.max(6, config.value.mapCrop.mapZoom + delta))
  draw()
}
...
function onPointerMove(e: PointerEvent): void {
  ...
  config.value.mapCrop.mapCenterLon -= (dx * mpp) / (111320 * Math.cos(mapCenterLat * (Math.PI / 180)))
